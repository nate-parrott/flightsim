...
@export var control_stick : ControlStick
...
func _ready():
	...
	throttle.connect("throttle_changed", Callable(self, "_on_throttle_changed"))
...
func setup():
	for module in modules:
		module.setup(self)
	engine_modules = find_modules_by_type("engine")
	steering_module = find_modules_by_type("steering").pop_front()
...
func _on_throttle_changed(level):
	send_to_engines("engine_set_power", [level])
	prop_target_rpm = level * 2000.0

func _process(delta):
	if not world_ref:
		return
	
	if joystick_left and joystick_left.output.y != 0:
		var throttle = joystick_left.output.y
		send_to_engines("engine_increase_power", [-0.4 * throttle * delta])
	
	#if joystick_right and steering_module:
		#var axis_z = joystick_right.output.x
		#steering_module.set_z(axis_z)
		#
		#var axis_x = joystick_right.output.y
		#steering_module.set_x(axis_x)
	
	if control_stick and steering_module:
		steering_module.set_z(remap(control_stick.pos.x, 0, 1, -1, 1))
		
		# Compute roll:
		var local_countergravity = -local_gravity_direction
		var current_roll = rad_to_deg(atan2(-local_countergravity.x, local_countergravity.y))
		var steer = clamp(remap(current_roll, -50, 50, 1, -1), -1, 1)
		steering_module.set_y(steer * 0.5)
		
		steering_module.set_x(remap(control_stick.pos.y, 0, 1, -1, 1))
...
var prop_target_rpm = 0.0
var prop_decel_rate = 0.2
var prop_acel_rate = 0.7
func _update_propeller(delta: float):
	propeller_rpm = lerp(propeller_rpm, prop_target_rpm, prop_acel_rate if prop_target_rpm > propeller_rpm else prop_decel_rate * delta)
	propeller.rotation.z += propeller_rpm / 60 * delta
