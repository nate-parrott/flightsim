...
func engine_start():
	...
	sfx_tween.tween_property(sfx_engine_loop, "volume_db", VolumeScale, 1.0)
...
func engine_set_power(value: float):
	...
	if sfx_tween:
		sfx_tween.kill()
	sfx_tween = create_tween()
	sfx_tween.tween_property(sfx_engine_loop, "volume_db", VolumeScale, 0.3).set_trans(Tween.TRANS_LINEAR)
